...
void WaterfallDown(int StartPos, int EndPos, byte Freq, byte ColourSelector)
{
  ...
  for (int i=0; i<totalLength; i++)
  {
    switch (ColourSelector) {
      ...
      case 1: strip.setPixelColor(StartPos+i, yWheel(history[StartPos+i])); break;
      case 2: strip.setPixelColor(StartPos+i, gWheel(history[StartPos+i])); break;
      case 3: strip.setPixelColor(StartPos+i, cWheel(history[StartPos+i])); break;
      case 4: strip.setPixelColor(StartPos+i, bWheel(history[StartPos+i])); break;
      case 5: strip.setPixelColor(StartPos+i, pWheel(history[StartPos+i])); break;
      default: strip.setPixelColor(StartPos+i, wWheel(history[StartPos+i])); break;
    }
  }
}
